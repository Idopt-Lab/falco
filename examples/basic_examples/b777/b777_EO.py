...
# right_aileron_component.parameters.actuate_angle = -left_aileron_component.parameters.actuate_angle
...
class B777Control(VehicleControlSystem):
    ...
    def __init__(self, elevator_component, aileron_right_component, aileron_left_component, rudder_component, symmetrical: bool = True):
        ...
        self.engine_left = PropulsiveControl(name='engine_left', throttle=0.1)
        self.engine_right = PropulsiveControl(name='engine_right', throttle=0.1)
        self.rudder = ControlSurface('rudder', lb=-20, ub=20, component=rudder_component)
...
B777_controls.yaw_control[0].deflection.set_as_design_variable(lower=np.deg2rad(-20), upper=np.deg2rad(20))
B777_controls.roll_control[0].deflection.set_as_design_variable(lower=np.deg2rad(-20), upper=np.deg2rad(20))
...
res7 = tf[1] * Drag_scaling
res7.name = 'Fy Force'
res7.set_as_constraint(lower=-1e-6, upper=1e-6)
...
res5 = tm[0] * Moment_scaling
res5.name = 'Mx Moment'
res5.set_as_constraint(lower=-1e-1, upper=1e-1)

res6 = tm[2] * Moment_scaling
res6.name = 'Mz Moment'
res6.set_as_constraint(lower=-1e-1, upper=1e-1)
...
t1 = time.time()
prob = CSDLAlphaProblem(problem_name='OEI_B777', simulator=sim)
optimizer = IPOPT(problem=prob, solver_options={'max_iter': 1500, 'tol': 1e-5})
...
print("Throttle")
for engine in B777_controls.throttle_control:
    print(engine.throttle.value)
print("Elevator Deflection (deg)")
print(B777_controls.pitch_control[0].deflection.value * 180 / np.pi)
print("Aileron Left Deflection (deg)")
print(B777_controls.roll_control[0].deflection.value * 180 / np.pi)
print("Aileron Right Deflection (deg)")
print(B777_controls.roll_control[1].deflection.value * 180 / np.pi)
print("Rudder Deflection (deg)")
print(B777_controls.yaw_control[0].deflection.value * 180 / np.pi)
print("Mach Number")
print(cruise_cond.parameters.mach_number.value)
print("Altitude (m)")
print(cruise_cond.parameters.altitude.value)
print("Velocity (m/s)")
print(cruise_cond.ac_states.VTAS.value)
print("Pitch Angle (deg)")
print(cruise_cond.ac_states.states.theta.value * 180 / np.pi)
print("Yaw Angle (deg)")
print(cruise_cond.ac_states.states.psi.value * 180 / np.pi)
print('Flight Path Angle (deg)')
print(cruise_cond.ac_states.gamma.value * 180 / np.pi)
