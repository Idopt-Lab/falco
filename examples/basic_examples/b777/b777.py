...
fd_axis = Axis(
    ...
    x=Q_(0, 'ft'),
    y=Q_(0, 'ft'),
    z=Q_(-1000, 'ft'),  # z is positive down in FD axis
    phi=Q_(0, 'deg'),
    theta=Q_(4, 'deg'),
    psi=Q_(0, 'deg'),
...
b777_mi = MassMI(axis=fd_axis,
                 ...
                 Ixz=Q_(0, 'kg*(m*m)'),
                 Iyz=Q_(0, 'kg*(m*m)'))
b777_mass_properties = MassProperties(mass=Q_(299370, 'kg'),
                                      inertia=b777_mi,
...
class B777Aerodynamics(Loads):
    ...
    def get_FM_refPoint(self, x_bar, u_bar):
        ...
        p = x_bar.state_vector.p
        q = x_bar.state_vector.q
        r = x_bar.state_vector.r
        theta = x_bar.state_vector.theta
        h = x_bar.state_vector.z

        da = u_bar.roll_control[0].deflection
        dr = u_bar.yaw_control[0].deflection
        de = u_bar.pitch_control[0].deflection
...
class B777SMTPropulsion(Loads):
    ...
    def get_FM_refPoint(self, x_bar, u_bar):
        ...
        axis = x_bar.axisaxis = x_bar.axis
        h = x_bar.state_vector.z
        M = x_bar.Mach
...
class B777SMTEngineCurve(csdl.CustomExplicitOperation):
    def __init__(self):
        ...
        propcurve_alt_data = eng_inp_data['alt']*1000
...
# B777_controls.elevator.deflection.set_as_design_variable(lower=-np.deg2rad(30), upper=np.deg2rad(30))
B777_controls.engine_right.throttle.set_as_design_variable(lower=0,
                                                        upper=1,scaler=1e-3)
B777_controls.engine_left.throttle.set_as_design_variable(lower=0,
                                                        upper=1,scaler=1e-3)
...
residual3 = tf[0]
residual3.name = 'Fx=0'
residual3.set_as_constraint(lower=-1e-2, upper=1e-2)
...
J.set_as_constraint(lower=-1e-6, upper=1e-6,scaler=1e3)
# J.set_as_objective()

TotalThrust = (B777_left_engine.get_FM_refPoint(cruise_cond.ac_states, B777_controls).F.vector + B777_right_engine.get_FM_refPoint(cruise_cond.ac_states, B777_controls).F.vector)[0]
TotalThrust.name = 'Total Thrust'
ThrustRequired = -B777_wing_aero.get_FM_refPoint(cruise_cond.ac_states, B777_controls).F.vector[0]
...
sim = csdl.experimental.JaxSimulator(
        recorder=recorder,
        gpu=False,
        additional_inputs=[cruise_cond.parameters.mach_number],
        additional_outputs=[TotalThrust, ThrustRequired,cruise_cond.ac_states.VTAS],
        derivatives_kwargs= {
            "concatenate_ofs" : True
        })

Trequireds = []
machs = np.arange(0.05, 0.9, 0.025)
vtas_list = []

for i, mach in enumerate(machs):
    sim[cruise_cond.parameters.mach_number] = mach

    sim.check_optimization_derivatives()

    t1 = time.time()
    prob = CSDLAlphaProblem(problem_name='trim_thrust', simulator=sim)
...
plt.figure()
plt.plot(machs, Trequireds, marker='s', linestyle='--', label='Required Thrust')
plt.xlabel('Mach Number')
plt.ylabel('Thrust (N)')
plt.title('Thrust vs. Mach')
plt.grid(True)
plt.legend()
plt.show()
