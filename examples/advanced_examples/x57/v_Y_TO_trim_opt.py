...
takeoff.parameters.pitch_angle.set_as_design_variable(lower=np.deg2rad(-10), upper=np.deg2rad(10),scaler=1e2) # in radians, this is the pitch angle range for the optimization
takeoff.parameters.flight_path_angle.set_as_design_variable(lower=np.deg2rad(-10), upper=np.deg2rad(10),scaler=1e2) # in radians, this is the pitch angle range for the optimization
...
with open('v_Y_TO_trim_opt.csv', 'w') as f:
