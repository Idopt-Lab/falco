...
takeoff.parameters.pitch_angle.set_as_design_variable(lower=np.deg2rad(-20), upper=np.deg2rad(20),scaler=1e2) # in radians, this is the pitch angle range for the optimization
takeoff.parameters.flight_path_angle.set_as_design_variable(lower=np.deg2rad(-20), upper=np.deg2rad(20),scaler=1e2) # in radians, this is the pitch angle range for the optimization
...
for left_engine, right_engine in zip(x57_controls.cm_engines_left, x57_controls.cm_engines_right):
    ...
    cm_throttle_diff.set_as_constraint(lower=-1e-6,upper=1e-6)  
...
VX_obj = (-takeoff.ac_states.gamma) * 1e2
...
with open('v_X_TO_trim_opt.csv', 'w') as f:
