...
sim = csdl.experimental.JaxSimulator(
    ...
    additional_outputs=[cruise.ac_states.VTAS, cruise.ac_states.alpha],
    derivatives_kwargs= {
        "concatenate_ofs" : True})


speeds = np.array([76.8909]) # X57 cruise speed in m/s
...
for i, alpha in enumerate(pitch_angles):
    ...
    t1 = time.time()
    prob = CSDLAlphaProblem(problem_name='CL_vs_alpha', simulator=sim)
    ...
    results_dict['Alpha'].append(cruise.ac_states.alpha.value[0])
...
results_df.to_csv(os.path.join(outputs_dir, 'CL_vs_alpha.csv'), index=False)


import matplotlib.pyplot as plt

plt.figure()
plt.plot(results_df['Alpha'] * 180/np.pi, results_df['CL'], marker='o', linestyle='-', label='CL')
