...
def py2md(config):
    ...
    for ex in glob.iglob(config['target'] + '**/ex_*.py', recursive=True):
        ...
        with open(ex[:-3]+'.md', 'w', encoding="utf-8") as g:
            g.write('# ' + title + '\n')
            g.write(desc + '\n\n')
            g.write('```python\n')
            g.write(code)
...
collections = {
   ...
   'copy_tutorials': {
      'driver': 'copy_folder',
      'source': '../tutorials', # Copy all tutorials
      'target': 'tutorials/',
      'ignore': [],
      'clean': True,            
      'final_clean': False,      # Keep files after build 
   },

   # Commented out examples collections since you only want tutorials
   # 'copy_examples': {
   #    'driver': 'copy_folder',
   #    'source': '../examples',  
   #    'target': 'examples/',
